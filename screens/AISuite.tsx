...
const AISuite: React.FC<AISuiteProps> = ({ navigate, t }) => {
  ...
  return (
    <div className="p-4 space-y-6 animate-in fade-in duration-500 h-full overflow-y-auto custom-scrollbar">
      <div className="space-y-1">
        <h2 className="text-3xl font-black text-[#eab308] italic uppercase tracking-tighter leading-none">Casillas Hub</h2>
        <div className="flex items-center gap-3">
           <div className="h-1 w-6 bg-[#eab308]/30"></div>
           <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em]">{t.active_industrial_intelligence || 'Inteligência Industrial Ativa'}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4">
        {tools.map(tool => (
          <button
            ...
            className={`w-full p-4 rounded-[2.5rem] border-2 flex items-center gap-4 transition-all active:scale-95 shadow-2xl relative overflow-hidden group ${tool.color}`}
          >
            <div className="size-16 rounded-[1.5rem] bg-black/40 flex items-center justify-center shrink-0 border border-white/10 shadow-inner overflow-hidden">
            ...
            <div className="flex-1 text-left min-w-0">
               <h3 className="font-black text-base tracking-tight uppercase italic leading-none text-white group-hover:text-[#eab308] transition-colors">
                 {tool.title}
               </h3>
               <p className="text-[8px] font-bold opacity-80 leading-tight uppercase tracking-widest mt-1.5 block">
                 {tool.desc}
               </p>
            </div>
      ...
      <div className="bg-[#1c1e22] p-4 rounded-[30px] border border-white/10 mt-2">
         <div className="flex items-center gap-4">
            <div className="size-10 rounded-xl bg-black/40 flex items-center justify-center text-[#eab308] shrink-0">
               <span className="material-symbols-outlined text-2xl">security</span>
            </div>
            <p className="text-[9px] font-black text-gray-400 leading-tight uppercase tracking-[0.1em]">
