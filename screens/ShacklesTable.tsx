...
const ShacklesTable: React.FC = () => {
  const [activeTab, setActiveTab] = useState<AccessoryType>('Curva');
  const [material, setMaterial] = useState<MaterialGrade>('Aço Liga');
  ...
  const tableData = useMemo(() => {
    if (activeTab === 'Olhal') {
      return EYEBOLTS_DATA.map(item => {
        ...
        if (material === 'Aço Liga') wll = item.wll_carbon * 1.5; // Aproximação Alloy vs Carbon
        if (material === 'Inox') wll = item.wll_carbon * 0.8;    // Aproximação Inox vs Carbon
      ...
      return SHACKLES_DATA.map(item => {
        ...
        if (material === 'Aço Carbono') wll = item.wll_alloy * 0.65;
        if (material === 'Inox') wll = item.wll_alloy * 0.55;
  ...
  const getHeaderInfo = () => {
    if (activeTab === 'Olhal') {
      return {
        title: 'Olhal de Suspensão',
        norm: 'DIN 580 / ISO 3266',
        desc: 'Para içamento vertical de cargas. Verifique o torque de aperto e a integridade da rosca.',
        icon: 'emergency_home'
      };
    }
    return {
      title: `Manilha ${activeTab}`,
      norm: 'G-209 / G-210',
      desc: 'Pino roscado com trava de segurança. Ideal para conexões flexíveis e cabos de aço.',
      icon: 'plumbing'
    };
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#1c1e22]">
      <div className="p-4 space-y-4">
        <h2 className="text-[#eab308] text-center font-black text-lg uppercase tracking-widest">Manilhas & Olhais</h2>

        {/* Tab Selector */}
        <div className="bg-[#121214] rounded-xl p-1 flex gap-1 border border-white/5 shadow-inner">
          {(['Curva', 'Reta', 'Olhal'] as AccessoryType[]).map(tab => (
            <button
              key={tab}
        ...
        <div className="relative">
          ...
          <input 
            type="text" 
            placeholder={activeTab === 'Olhal' ? "Buscar Bitola (M8, M12...)" : "Buscar Medida (1/2\", 3/4\"...)"}
            value={search}
        ...
        <div className="flex gap-2 p-1 bg-[#121214] rounded-full border border-white/5">
           {(['Aço Liga', 'Aço Carbono', 'Inox'] as MaterialGrade[]).map(m => (
             <button 
               key={m}
        ...
        <div className="flex justify-between items-center px-1">
           <h3 className="text-[#eab308] text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
             ...
             Tabela de Cargas ({material})
           </h3>
           <span className="text-gray-700 text-[8px] font-black uppercase tracking-tighter">Unidade: Toneladas (t)</span>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 bg-[#1c1e22]/95 backdrop-blur-md z-10">
            <tr className="text-[9px] font-black text-gray-600 uppercase tracking-widest border-b border-white/5">
              <th className="p-4">{activeTab === 'Olhal' ? 'Rosca' : 'Diâm. (pol)'}</th>
              <th className="p-4 text-[#eab308]">Carga (WLL)</th>
              <th className="p-4 text-center">{activeTab === 'Olhal' ? 'Ø Int (mm)' : 'A (mm)'}</th>
              <th className="p-4 text-center">{activeTab === 'Olhal' ? 'Ø Ext (mm)' : 'B (mm)'}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {filtered.length > 0 ? filtered.map((row) => (
              <tr key={row.id} className="hover:bg-white/5 transition-colors group">
                ...
                <td className="p-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[8px] text-gray-600 font-bold uppercase tracking-widest">Carga Útil</span>
                  </div>
                </td>
              ...
              <tr>
                <td colSpan={4} className="p-10 text-center text-gray-700 text-xs font-bold uppercase italic">Nenhum dado encontrado</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="p-4 bg-[#121214] border-t border-white/5">
         <div className="bg-orange-500/5 border border-orange-500/20 rounded-2xl p-4 flex gap-4 animate-pulse">
            ...
            <div className="space-y-1">
               <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest">Protocolo de Segurança 5:1</p>
               <p className="text-[10px] text-orange-500/80 font-bold leading-tight">
                 Cargas calculadas com fator de segurança. {activeTab === 'Olhal' ? 'A carga do Olhal reduz em 30% para içamentos laterais (ângulos > 45°).' : 'Inspecione trincas ou deformações no pino antes do uso.'}
               </p>
            </div>
         </div>
      </div>
