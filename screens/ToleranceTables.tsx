...
const ToleranceTables: React.FC = () => {
    ...
    <div className="flex flex-col h-full bg-[#1c1e22]">
      <div className="p-4 space-y-6">
        <div className="flex bg-[#121214] rounded-xl p-1 border border-white/5">
          {['Eixos', 'Diâmetros', 'Chavetas'].map((t) => (
            <button
              key={t}
              onClick={() => setActiveTab(t)}
              className={`flex-1 py-3 text-xs font-bold rounded-lg transition-all ${
                activeTab === t ? 'bg-[#eab308] text-[#121214]' : 'text-gray-500'
              }`}
            >
              {t}
            </button>
        ...
        <div className="bg-[#252930] rounded-2xl p-5 border border-white/5 space-y-5 shadow-xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">Dimensão (mm)</label>
              <div className="bg-[#121214] rounded-xl p-3 border border-white/10 flex items-center justify-between">
                ...
                />
                <span className="text-gray-600 text-[10px] font-bold">mm</span>
              </div>
            </div>
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">Classe</label>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <h3 className="text-white font-black text-base uppercase tracking-tight">Resultados</h3>
          ...
          <div className="bg-gradient-to-br from-[#121214] to-black rounded-3xl p-6 border border-[#eab308]/20 shadow-2xl relative overflow-hidden">
             <div className="absolute top-4 right-4 bg-[#eab308]/5 text-[#eab308] text-[10px] font-black px-3 py-1 rounded-full border border-[#eab308]/20 uppercase">Eixo Padrão</div>
             ...
             <div className="grid grid-cols-2 gap-8">
                <div className="space-y-1">
                  <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">Máximo (mm)</p>
                ...
                <div className="space-y-1">
                  <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">Mínimo (mm)</p>
        ...
        <div className="bg-[#252930] rounded-2xl p-5 border border-white/5 space-y-4">
           <div className="flex justify-between items-center">
              <h4 className="text-white font-black text-[10px] uppercase tracking-[0.2em] flex items-center gap-2">
                ...
                Visualização de Zona
              </h4>
              <button className="text-gray-500 text-[10px] font-bold uppercase hover:text-white transition-colors">Detalhes</button>
           </div>
           
           <div className="relative h-20 flex flex-col justify-center">
              <div className="absolute top-0 left-0 text-[8px] font-bold text-gray-600">NOMINAL</div>
        ...
        <div className="space-y-3 pb-8">
           <h4 className="text-[#eab308] text-[10px] font-black uppercase tracking-widest ml-1">Valores Próximos ({isoClass})</h4>
           <div className="bg-[#121214] rounded-2xl border border-white/5 overflow-hidden">
              <div className="grid grid-cols-3 p-4 text-[9px] font-black text-gray-600 uppercase tracking-widest border-b border-white/5">
                 <span>Dimensão</span>
                 <span className="text-center">Sup. (mm)</span>
                 <span className="text-right">Inf. (mm)</span>
              </div>
              {[
                { r: '30 - 40 mm', s: '0', i: '-25' },
      ...
      <nav className="sticky bottom-0 bg-[#1c1e22] border-t border-white/5 p-2 flex justify-around items-center safe-pb">
         {['Calcular', 'Tabelas', 'Histórico', 'Ajustes'].map(n => (
           <button key={n} className="flex flex-col items-center gap-1 p-2">
              <span className={`material-symbols-outlined text-xl ${n === 'Calcular' ? 'text-[#eab308] filled' : 'text-gray-600'}`}>
                {n === 'Calcular' ? 'calculate' : n === 'Tabelas' ? 'list_alt' : n === 'Histórico' ? 'history' : 'settings'}
              </span>
              <span className={`text-[8px] font-black uppercase tracking-widest ${n === 'Calcular' ? 'text-[#eab308]' : 'text-gray-600'}`}>{n}</span>
           </button>
         ))}
      </nav>
    </div>
